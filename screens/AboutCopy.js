...
TaskManager.defineTask(TASK_SHOW_ALERT, ({ data: { locations }, error }) => {
	if (error) {
		console.error(error);
		return;
	}
	...
	AsyncStorage.getItem("listData", (err, data) => {
		...
		try {
			...
			const currentCheckDengue = checkDistance(listPatientInformation, listDengueLocationItem, latNew, longNew);

			AsyncStorage.getItem("lastestCheckDengue").then(res => {
				if (res !== null && res !== undefined) {
					if (res === "true") {
						if (currentCheckDengue === false) {
...
function About(props) {
	...
	const startTaskServiceBackgroundWarningDengue = () => {
		...
		Location.startLocationUpdatesAsync(TASK_SHOW_ALERT, {
			...
			distanceInterval: 0,
			foregroundService: {
				notificationTitle: "Dengue Alert",
				notificationBody: "Ứng dụng Dengue Alert đang hoạt động"
			}
		});
	};
	...
	const registerForPushNotificationsAsync = async () => {
		...
		// 	if (!locationEnable && locationStatus === "granted") {
		// 		isNotify = 1;
		// 		console.log(checkNotifyWhenGetGPS + "GPS");
	...
	useEffect(() => {
		...
		if (isLocation && isNetwork) {
			handleNotification();
			startTaskServiceBackgroundWarningDengue();
		}
	}, [isNetwork, isLocation]);
